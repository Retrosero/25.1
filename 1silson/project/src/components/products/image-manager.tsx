...
import { fetchProductImages } from '../../lib/xml';
...
export function ImageManager({
  ...
  useEffect(() => {
    const loadXmlImages = async () => {
      if (xmlSettings.productImagesUrl && productCode) {
        setIsLoading(true);
        setError(null);

        try {
          const xmlImages = await fetchProductImages(
            xmlSettings.productImagesUrl,
            productCode.trim()
          );

          if (xmlImages.length > 0) {
            xmlImages.forEach((img) => {
              if (!images.some((existing) => existing.url === img.url)) {
                onAddImage(img.url);
              }
            });
            console.log(`${xmlImages.length} resim yüklendi:`, xmlImages);
          } else {
            console.log('Ürün kodu için resim bulunamadı:', productCode);
          }
        } catch (err) {
          setError('XML resimlerini yüklerken bir hata oluştu');
  ...
  }, [xmlSettings.productImagesUrl, productCode, images, onAddImage]);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      onAddFile(file);
      e.target.value = ''; // Aynı dosyayı tekrar seçebilmek için temizlenir.
    }
  };
