...
export function DeleteConfirmation({ type, onConfirm, onCancel }: DeleteConfirmationProps) {
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-md">
        <div className="flex items-center gap-3 p-4 border-b border-gray-200 dark:border-gray-700">
          <AlertTriangle className="w-5 h-5 text-red-500" />
          <h3 className="font-medium">Silme Onayı</h3>
        </div>

        <div className="p-4">
          <p>Bu {type === 'note' ? 'notu' : type === 'payment' ? 'tahsilatı' : 'tediyeyi'} silmek istediğinizden emin misiniz?</p>
