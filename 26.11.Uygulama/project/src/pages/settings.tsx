...
import { syncProductImages } from "../lib/xml";
import { toast } from 'react-toastify'; // If you're using a toast notification library
...
export function SettingsPage() {
  ...
  const handleSyncImages = async () => {
    if (!xmlSettings.productImagesUrl) {
      alert('Lütfen bir XML URL\'si girin.');
      return;
    }
    setIsSyncing(true);
    try {
      const result = await syncProductImages(xmlSettings.productImagesUrl);
      if (result.success) {
      ...
      console.error(error);
      toast.error('Resimler senkronize edilirken bir hata oluştu.');
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <div className="max-w-2xl space-y-6">
        ...
        {activeTab === 'xml' && (
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <h2 className="text-lg font-semibold mb-4">XML Ayarları</h2>
            
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">
                  Ürün Resimleri XML Adresi
                </label>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={xmlSettings.productImagesUrl}
                    onChange={(e) => setXmlSettings({ productImagesUrl: e.target.value })}
                  ...
                  >
                    {isSyncing ? 'Senkronize Ediliyor...' : 'Resimleri Kaydet'}
                  </button>
                </div>
                <p className="text-sm text-gray-500 mt-2">
                  Bu XML adresi ürün resimlerini otomatik olarak yüklemek için kullanılacaktır.
                </p>
              </div>
            </div>
          </div>
